...
def imageprepare(index):
    ...
    img = cv2.imread("D:/DeepLearning/digitalRecognition/digitalData/" + str(index) + ".bmp", -1)
...
with tf.Session() as sess:
    for i in range(20):
        ...
        sess.run(init_op)
        saver = tf.train.import_meta_graph('D:/DeepLearning/digitalRecognition/modelGallery/defineGraphTF/MyModel.ckpt-1000.meta')
        saver.restore(sess, tf.train.latest_checkpoint('D:/DeepLearning/digitalRecognition/modelGallery/defineGraphTF/'))
        # print ("Model restored.")

        prediction = tf.argmax(y_conv, 1)
        # prediction2 = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)

        predint = prediction.eval(feed_dict={x: [result], keep_prob: 1.0}, session=sess)
        # predint2 = prediction2.eval(feed_dict={x: [result], keep_prob: 1.0}, session=sess)
